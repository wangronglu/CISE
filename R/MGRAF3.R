...
#'   Default is maxit = 5.
#'
#' @return A list is returned containing the ingredients below from M-GRAF3
...
#' @examples
#' data(A)
#' res = MGRAF3(A=A, K=5, tol=0.01, maxit=5)
#'
#' @import gdata
#' @import Matrix
#' @import glmnet
...
## A_i ~ Bernoulli(\Pi_i) logit(\Pi_i) = Z + D_i = Z + Q %*% \Lambda_i %*% t(Q) Q
## is the same for all subjects The algorithm iterate between the following steps
## until convergence 1. Given Z, \Lambda_i, solve for Q by seting each column k of Q
## at the first eigenvector of sum_{i=1}^{n} [\lambda_{ik}(A_i-P_0)] corresponding
## to the largest eigenvalue, where P_0 = 1/(1+exp(-Z)) 2. Given Q, solve Z and
## \Lambda_i by logistic regression

MGRAF3 = function(A, K, tol, maxit) {
    # A: VxVxn array storing n symmetric adjacency matrices (VxV).  K: latent dimension
    # of each D_i tol: convergence threshold for CISE algorithm. CISE iteration
    # continues until the absolute percent change in joint log-likelihood is smaller
    # than this value. Default is 0.01 maxit: maximum number of iterations in CISE.
    # Default is 5.
    ...
    ######################################################################################### SOLVE Q ----------------------------------------------- create intermediate
    ######################################################################################### matrices W_k = = sum_{i=1}^n {lambda_{ik}* [A_i - P0] } -------------------
    ...
    ## an array of lower-triangle of principal component matrices for computation
    ## convenience
    M = apply(Q, 2, function(x) {
        lowerTriangle(x %*% t(x))
    })  # LxK
    ...
    ###################################################################################### TUNE PENALTY PARAMETER LAMBDA IN GLMNET CONSTRUCT Y
    ###################################################################################### ----------------------------------------------------------------------
    y = factor(c(A_LT))
    ...
    for (st in 1:maxit) {
        ...
        #### compute joint log-likelihood -------------------------------------------------- an
        #### array of lower-triangle of principal component matrices for computation
        #### convenience
        M = apply(Q, 2, function(x) {
            lowerTriangle(x %*% t(x))
        })  # LxK
    ...
    results = list(Z = Z_best, Lambda = Lambda_best, Q = Q_best, LL_max = LL_max, LL = LL_seq)
    return(results)
}
